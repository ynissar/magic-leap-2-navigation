...
// Convert an MLTransform (position in metres + unit quaternion) to a 4×4
// homogeneous matrix suitable for use as a camera-to-world transform in OpenCV
// (CV_32F, row-major).
cv::Mat MLTransformToCvMat(const MLTransform &t) {
  float qx = t.rotation.x, qy = t.rotation.y, qz = t.rotation.z,
        qw = t.rotation.w;
...
const char *
GetMLDepthCameraFrameTypeString(const MLDepthCameraFrameType &frame_type) {
  switch (frame_type) {
  case MLDepthCameraFrameType_LongRange:
    return "LongRange";
  case MLDepthCameraFrameType_ShortRange:
    return "ShortRange";
  default:
    return "Error";
  }
...
const char *GetMLDepthCameraStreamString(const MLDepthCameraStream &stream) {
  switch (stream) {
  case MLDepthCameraStream_LongRange:
    return "LongRange";
  case MLDepthCameraStream_ShortRange:
    return "ShortRange";
  default:
    return "Error";
  }
...
const char *GetMLDepthCameraFlagsString(const MLDepthCameraFlags &flag) {
  switch (flag) {
  case MLDepthCameraFlags_DepthImage:
    return "DepthImage";
  case MLDepthCameraFlags_Confidence:
    return "Confidence";
  case MLDepthCameraFlags_AmbientRawDepthImage:
    return "AmbientRawDepthImage";
  case MLDepthCameraFlags_RawDepthImage:
...
const char *
GetMLDepthCameraFrameRateString(const MLDepthCameraFrameRate &flag) {
  switch (flag) {
  case MLDepthCameraFrameRate_1FPS:
    return "1 Hz";
  case MLDepthCameraFrameRate_5FPS:
    return "5 Hz";
  case MLDepthCameraFrameRate_25FPS:
    return "25 Hz";
  case MLDepthCameraFrameRate_30FPS:
    return "30 Hz";
  case MLDepthCameraFrameRate_50FPS:
    return "50 Hz";
  case MLDepthCameraFrameRate_60FPS:
    return "60 Hz";
  default:
    return "Error";
  }
...
public:
  DepthCameraApp(struct android_app *state)
      : Application(
            state,
            std::vector<std::string>{"com.magicleap.permission.DEPTH_CAMERA"},
            USE_GUI),
        cam_context_(ML_INVALID_HANDLE),
        last_dcam_frametype_(MLDepthCameraFrameType_Ensure32Bits),
        last_dcam_intrinsics_{}, last_dcam_frame_number_(-1), last_dcam_pose_{},
        is_preview_invalid_(false), cap_idx_(0) {
    ...
    // Initialize limit values for shaders. These values are BY NO MEANS
    // suggested ranges for depth camera data.
    ...
    distance_limit_[GetIndexFromCameraFlag(MLDepthCameraFlags_DepthImage)] =
        7.5f;
    ...
    max_distance_[GetIndexFromCameraFlag(MLDepthCameraFlags_Confidence)] =
        100.0f;
    distance_limit_[GetIndexFromCameraFlag(MLDepthCameraFlags_Confidence)] =
        100.0f;
    legend_unit_[GetIndexFromCameraFlag(MLDepthCameraFlags_Confidence)] = '%';

    min_distance_[GetIndexFromCameraFlag(
        MLDepthCameraFlags_AmbientRawDepthImage)] = 5.0f;
    max_distance_[GetIndexFromCameraFlag(
        MLDepthCameraFlags_AmbientRawDepthImage)] = 2000.0f;
    distance_limit_[GetIndexFromCameraFlag(
        MLDepthCameraFlags_AmbientRawDepthImage)] = 2000.0f;
    legend_unit_[GetIndexFromCameraFlag(
        MLDepthCameraFlags_AmbientRawDepthImage)] = ' ';

    min_distance_[GetIndexFromCameraFlag(MLDepthCameraFlags_RawDepthImage)] =
        5.0f;
    max_distance_[GetIndexFromCameraFlag(MLDepthCameraFlags_RawDepthImage)] =
        2000.0f;
    distance_limit_[GetIndexFromCameraFlag(MLDepthCameraFlags_RawDepthImage)] =
        2000.0f;
    ...
    camera_settings_.stream_configs[MLDepthCameraFrameType_LongRange].flags =
        MLDepthCameraFlags_DepthImage | MLDepthCameraFlags_Confidence |
        MLDepthCameraFlags_AmbientRawDepthImage |
        MLDepthCameraFlags_RawDepthImage;
    camera_settings_.stream_configs[MLDepthCameraFrameType_ShortRange].flags =
        MLDepthCameraFlags_DepthImage | MLDepthCameraFlags_Confidence |
        MLDepthCameraFlags_AmbientRawDepthImage |
        MLDepthCameraFlags_RawDepthImage;
  ...
  void OnStart() override {
    ...
    color_map_tex_ = Registry::GetInstance()->GetResourcePool()->LoadTexture(
        "depth_gradient.png", GL_RGBA);
    conf_map_tex_ = Registry::GetInstance()->GetResourcePool()->LoadTexture(
        "confidence_gradient.png", GL_RGBA);
    ...
    SetHeadHandle(head_tracker); ///< This will cause Application class to
                                 ///< destroy HT handle on app exit.

    const auto head_pose_opt = GetHeadPoseOrigin();
    if (!head_pose_opt.has_value()) {
      ALOGE("No head pose available at application start! For best experience, "
            "start the application with the device on.");
    }
    const Pose head_pose = head_pose_opt.value_or(GetRoot()->GetWorldPose())
                               .HorizontalRotationOnly();
    const Pose gui_offset(glm::vec3(
        .25f, 0.f, -2.f)); //> Make gui not obscure the preview too much
...
private:
  ...
  void UpdateGui() {
    ...
    if (gui.BeginDialog("Depth Camera", &is_running,
      ...
      ImGui::Text("Marker Overlay");
      {
        ...
        snprintf(det_hdr, sizeof(det_hdr), "Detected (%zu)##det",
                 detected_markers_.size());
        snprintf(rej_hdr, sizeof(rej_hdr), "Rejected (%zu)##rej",
                 rejected_blobs_.size());

        if (ImGui::CollapsingHeader(det_hdr)) {
          for (size_t m = 0; m < detected_markers_.size(); m++) {
            const auto &mk = detected_markers_[m];
            ImGui::Text(
                "  [%zu] px(%.1f,%.1f) 3D(%.3f,%.3f,%.3f)m area=%d inten=%.0f",
        ...
        if (ImGui::CollapsingHeader(rej_hdr)) {
          for (size_t m = 0; m < rejected_blobs_.size(); m++) {
            ...
            const char *reason_str =
                rb.reason == ml::marker_detection::RejectionReason::OutOfBounds
                    ? "OOB"
                : rb.reason ==
                        ml::marker_detection::RejectionReason::InvalidDepth
                    ? "Depth"
                    : "Area";
            ImGui::TextColored(
                ImVec4(0.2f, 1.f, 0.2f, 1.f),
                "  [%zu] %s px(%.1f,%.1f) area=%d inten=%.0f depth=%.3fm", m,
                reason_str, rb.centroid_pixel.x, rb.centroid_pixel.y,
                rb.area_pixels, rb.intensity, rb.depth_m);
            if (rb.reason ==
                ml::marker_detection::RejectionReason::AreaMismatch) {
              ImGui::Text("       expected=%.1f [%.1f..%.1f] px",
                          rb.expected_area, rb.expected_area_min,
                          rb.expected_area_max);
            }
          }
        }
      }
      drawToolCapturePanel();
      drawTuningPanel();
    }
    gui.EndDialog();
    gui.EndUpdate();
  ...
  void drawStreamInfoPanel() {
    ImGui::NewLine();
    ImGui::Text("Basic frame information");
    {
      ImGui::Text("\tCamera position xyz: (%.2f, %.2f, %.2f)",
                  last_dcam_pose_.position.x, last_dcam_pose_.position.y,
                  last_dcam_pose_.position.z);
      ImGui::Text("\tCamera rotation xyzw: (%.2f, %.2f, %.2f, %.2f)",
                  last_dcam_pose_.rotation.x, last_dcam_pose_.rotation.y,
                  last_dcam_pose_.rotation.z, last_dcam_pose_.rotation.w);
      ImGui::Text("\tFrame number: %ld", last_dcam_frame_number_);
      ImGui::Text("\tFrame time since device boot: %s",
                  last_dcam_timestamp_str_);
      ImGui::Text("\tFrame type: %s",
                  GetMLDepthCameraFrameTypeString(last_dcam_frametype_));
    }

    DrawIntrinsicDetails("Intrinsics:", last_dcam_intrinsics_);
  }

  void drawStreamSettingsPanel() {
    ImGui::NewLine();
    ImGui::Separator();
    ImGui::Text("Settings");
    {
      const bool are_settings_updated = update_settings_future_.valid();
      if (are_settings_updated)
        ImGui::BeginDisabled();

      ImGui::Text("Data type:");
      ImGui::SameLine();
      if (ImGui::BeginTable("camera_flags_table", 2,
      ...
      ImGui::SameLine();
      DrawModeRadioButton(MLDepthCameraStream_ShortRange);
      ImGui::SameLine();
      DrawModeRadioButton(MLDepthCameraStream_LongRange);

      ImGui::NewLine();
      DrawControlSettings(
          static_cast<MLDepthCameraStream>(camera_settings_.streams));
  ...
  void drawTuningPanel() {
    ...
    if (ImGui::CollapsingHeader("Tuning")) {
      ...
      if (ImGui::TreeNode("Marker Detection")) {
        ...
        ImGui::SliderFloat("Sphere radius (mm)", &tune_sphere_radius_mm_, 1.f,
                           20.f);
        ImGui::Checkbox("Ambient subtraction", &tune_ambient_subtraction_);
        ImGui::Checkbox("Log depth scaling", &tune_log_depth_scaling_);
        ImGui::SliderInt("Depth log every N frames", &tune_log_every_n_frames_,
                         1, 120);
        ImGui::Checkbox("Log pairwise distances",
                        &tune_log_pairwise_distances_);
        ImGui::SliderInt("Pairwise log every N frames",
                         &tune_log_pairwise_every_n_frames_, 1, 120);
        if (ImGui::Button("Reset depth baseline")) {
          request_reset_depth_baseline_ = true;
        }
        if (ImGui::IsItemHovered()) {
          ImGui::SetTooltip(
              "Clears mean-z baseline for ratio_z/dz on next scaled log.");
        }
        ...
        if (ImGui::SliderInt("Gaussian blur kernel",
                             &tune_gaussian_kernel_size_, 1, 15)) {
          tune_gaussian_kernel_size_ =
              std::max(1, tune_gaussian_kernel_size_ | 1);
        }
        if (ImGui::SliderInt("Morph kernel size", &tune_morph_kernel_size_, 1,
                             15)) {
      ...
      if (ImGui::TreeNode("Graph Search Tolerances")) {
        if (ImGui::SliderFloat("Side tol (mm)", &tune_tolerance_side_, 0.5f,
                               20.f))
          tool_tracker_.SetTolerances(tune_tolerance_side_, tune_tolerance_avg_,
                                      tune_tolerance_rel_);
        if (ImGui::SliderFloat("Avg tol (mm)", &tune_tolerance_avg_, 0.5f,
                               20.f))
          tool_tracker_.SetTolerances(tune_tolerance_side_, tune_tolerance_avg_,
                                      tune_tolerance_rel_);
        ...
        if (ImGui::IsItemHovered())
          ImGui::SetTooltip(
              "Relative per-side tolerance as a fraction of expected\n"
              "distance. Compensates for depth-scale error at different\n"
              "operating distances. Effective tol = max(Side tol, Rel tol * "
              "side).");
        ImGui::TreePop();
      }
      ...
      if (ImGui::TreeNode("Pose Smoothing")) {
        if (ImGui::SliderFloat("Position alpha", &tune_lowpass_position_, 0.f,
                               1.f))
          tool_tracker_.SetLowpassFactors(tune_lowpass_position_,
                                          tune_lowpass_rotation_);
        if (ImGui::SliderFloat("Rotation alpha", &tune_lowpass_rotation_, 0.f,
                               1.f))
          tool_tracker_.SetLowpassFactors(tune_lowpass_position_,
                                          tune_lowpass_rotation_);
        ImGui::TreePop();
      ...
      if (ImGui::TreeNode("Kalman Filter")) {
        ...
        kalman_changed |= ImGui::SliderFloat(
            "Measurement noise", &tune_kalman_measurement_, 0.01f, 10.f);
        kalman_changed |=
            ImGui::SliderFloat("Position noise", &tune_kalman_position_, 1e-6f,
                               1.f, "%.6f", ImGuiSliderFlags_Logarithmic);
        kalman_changed |= ImGui::SliderFloat(
            "Velocity noise", &tune_kalman_velocity_, 0.01f, 50.f);
        if (kalman_changed)
          tool_tracker_.ResetKalmanFilters(tune_kalman_measurement_,
                                           tune_kalman_position_,
                                           tune_kalman_velocity_);
        ImGui::TreePop();
      }
    }
  }

  void drawToolCapturePanel() {
    ImGui::Separator();
    ImGui::NewLine();
    ImGui::Text("Tool Tracking");
    {
      ...
      for (const auto &name : tool_names) {
        ...
        if (valid) {
          ImGui::Text("  pos (%.3f, %.3f, %.3f) m  [world]", tf.at<float>(0, 0),
                      tf.at<float>(1, 0), tf.at<float>(2, 0));
          ImGui::Text("  rot (%.3f, %.3f, %.3f, %.3f)", tf.at<float>(3, 0),
    ...
    if (ImGui::CollapsingHeader("Tool Capture")) {
      if (capture_state_ == CaptureState::Idle) {
        ...
        ImGui::SliderFloat("Sphere radius (mm)", &capture_sphere_radius_mm_,
                           1.f, 20.f);
        ImGui::SliderFloat("Max sphere distance (mm)",
                           &capture_max_pair_dist_mm_, 50.f, 500.f);
        ...
        float progress =
            (float)capture_frames_collected_ / (float)capture_num_frames_;
        ImGui::ProgressBar(progress);
        ImGui::Text("Collected: %d / %d   Skipped: %d",
                    capture_frames_collected_, capture_num_frames_,
                    capture_frames_skipped_);
        int cur_detect = (int)detected_markers_.size();
        if (cur_detect != capture_expected_spheres_) {
          ImGui::TextColored(ImVec4(1.f, 1.f, 0.f, 1.f),
                             "Detected %d markers (expected %d)", cur_detect,
                             capture_expected_spheres_);
        } else {
        ...
        if (ImGui::Button("Register Tool")) {
          ...
          tool_tracker_.AddTool(capture_result_mm_, capture_sphere_radius_mm_,
                                capture_tool_name_,
                                std::max(3, capture_expected_spheres_ - 1));
  ...
  void CreateGuiGroup(uint8_t idx) {
    ...
    if (ImGui::CollapsingHeader(GetMLDepthCameraFlagsString(camera_flag))) {
      ...
      const bool is_slider_needed =
          camera_flag != MLDepthCameraFlags_Confidence;
      if (is_slider_needed) {
        ImGui::NewLine();
        ImGui::Text("Shader valid values range:");
        if (ImGui::SliderFloat("Minimum", &min_distance_[idx], 0.0,
                               max_distance_[idx])) {
          UpdateDepthBound(idx, true);
          SetLegendText(idx);
        }
        if (ImGui::SliderFloat("Maximum", &max_distance_[idx],
                               min_distance_[idx], distance_limit_[idx])) {
          UpdateDepthBound(idx, false);
          SetLegendText(idx);
        }
      }
      ImGui::PopID();
  ...
  void UpdateDepthBound(const uint8_t idx, const bool is_min_bound) {
    auto depth_mat =
        std::static_pointer_cast<DepthMaterial>(shader_materials_[idx]);
    auto conf_mat =
        std::static_pointer_cast<ConfidenceMaterial>(shader_materials_[idx]);
    if (depth_mat) {
      if (is_min_bound) {
  ...
  void DrawControlSettings(MLDepthCameraStream stream) {
    ...
    if (ImGui::BeginCombo("##framerate", frame_rates[cap_idx_])) {
      for (size_t i = 0; i < frame_rates.size(); i++) {
        ...
        if (ImGui::Selectable(frame_rates[i], is_selected)) {
          camera_settings_.stream_configs[frame_type].frame_rate =
              stream_caps_[frame_type][i].frame_rate;
          cap_idx_ = i;

          // After switching frame_rate, check exposure against current
          // capabilities
          const auto &stream_cap = stream_caps_[frame_type][cap_idx_];
          auto &exposure = camera_settings_.stream_configs[frame_type].exposure;
          exposure = std::clamp(exposure, stream_cap.min_exposure,
                                stream_cap.max_exposure);
        }
        if (is_selected) {
    ...
    ImGui::Text("Exposure:");
    ImGui::SameLine();
    if (ImGui::SliderInt("##exposure", &exposure, stream_cap.min_exposure,
                         stream_cap.max_exposure, "%dus")) {
      camera_settings_.stream_configs[frame_type].exposure = exposure;
    }
  }

  void DrawModeRadioButton(MLDepthCameraStream stream) {
    if (ImGui::RadioButton(GetMLDepthCameraStreamString(stream),
      ...
      cap_idx_ = FindFrameRateIdx(
          frame_type, camera_settings_.stream_configs[frame_type].frame_rate);
    }
  }

  void DrawDataTypeCheckbox(MLDepthCameraFlags flag) {
    const auto config_idx = GetFrameTypeFromStream(
        static_cast<MLDepthCameraStream>(camera_settings_.streams));
    auto &flags_ref = camera_settings_.stream_configs[config_idx].flags;
    if (ImGui::CheckboxFlags(GetMLDepthCameraFlagsString(flag), &flags_ref,
                             flag)) {
      StartUpdateSettingsAsync();
      const auto flag_idx = GetIndexFromCameraFlag(flag);
      const bool is_enabled = flags_ref & flag;

      // If we're disabling depth image, clear the depth texture, so our
      // confidence shader doesn't reuse old depth values.
      if (!is_enabled && flag == MLDepthCameraFlags_DepthImage) {
        glBindTexture(GL_TEXTURE_2D, texture_id_[flag_idx]);
        glTexImage2D(GL_TEXTURE_2D, 0, GL_R32F, texture_width_[flag_idx],
                     texture_height_[flag_idx], 0, GL_RED, GL_FLOAT,
                     blank_depth_tex_.data());
  ...
  void DrawIntrinsicDetails(const char *label,
                            const MLDepthCameraIntrinsics &params) {
    if (ImGui::CollapsingHeader(label)) {
      ImGui::Text("Camera width: %d", params.width);
      ImGui::Text("Camera height: %d", params.height);
      ImGui::Text("Camera focal length: %.4f %.4f", params.focal_length.x,
                  params.focal_length.y);
      ImGui::Text("Camera principal point: %.4f %.4f", params.principal_point.x,
                  params.principal_point.y);
      ImGui::Text("Camera field of view: %.4f", params.fov);
      ImGui::Text("Camera distortion params k1, k2, p1, p2, k3:\n\t\t%.4f %.4f "
                  "%.4f %.4f %.4f",
                  params.distortion[0], params.distortion[1],
                  params.distortion[2], params.distortion[3],
                  params.distortion[4]);
    }
  }
  ...
  void UpdatePreview() {
    ...
    UNWRAP_MLRESULT(MLSnapshotGetTransform(
        snapshot, &head_static_data_.coord_frame_head, &head_transform));
  ...
  std::shared_ptr<Mesh>
  CreateMarkerPointMeshInternal(const std::vector<TMarker> &markers,
                                uint8_t quad_index) {
    ...
    auto mesh =
        std::make_shared<Mesh>(Buffer::Category::Dynamic, GL_UNSIGNED_INT);
  ...
  template <typename TMarker, typename TNodeContainer>
  void
  UpdateOverlayNodes(uint8_t quad_index, const std::vector<TMarker> &markers,
                     TNodeContainer &overlay_nodes, const glm::vec3 &color) {
    ...
    if (markers.empty() || !show_marker_overlay_) {
      if (overlay_nodes[quad_index]) {
        parent_node->RemoveChild(overlay_nodes[quad_index]);
      }
      return;
    }
    ...
    // Create material
    auto marker_material = std::make_shared<MarkerOverlayMaterial>(color, 0.9f);

    // Create or update node
    if (!overlay_nodes[quad_index]) {
      auto renderable =
          std::make_shared<RenderableComponent>(marker_mesh, marker_material);
      overlay_nodes[quad_index] = std::make_shared<Node>();
      overlay_nodes[quad_index]->AddComponent(renderable);
      parent_node->AddChild(overlay_nodes[quad_index]);
    } else {
      auto renderable = overlay_nodes[quad_index]
                            ->template GetComponent<RenderableComponent>();
  ...
  void UpdateMarkerOverlay(
      uint8_t quad_index,
      const std::vector<ml::marker_detection::DetectedMarker> &markers) {
    UpdateOverlayNodes(quad_index, markers, marker_overlay_nodes_,
                       marker_color_);
  }

  void UpdateRejectedOverlay(
      uint8_t quad_index,
      const std::vector<ml::marker_detection::RejectedBlob> &blobs) {
    const glm::vec3 rejected_color(0.0f, 1.0f, 0.0f);
    UpdateOverlayNodes(quad_index, blobs, rejected_overlay_nodes_,
                       rejected_color);
  }
  ...
  void SetupPreview() {
    ...
    for (uint8_t i = 0; i < texs.size(); ++i) {
      ...
      glTexImage2D(GL_TEXTURE_2D, 0, GL_R32F, texture_width_[i],
                   texture_height_[i], 0, GL_RED, GL_FLOAT, nullptr);
      glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_NEAREST);
      glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_LINEAR);
      glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_S, GL_CLAMP_TO_EDGE);
      glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_T, GL_CLAMP_TO_EDGE);
      texs[i] = std::make_shared<Texture>(
          GL_TEXTURE_2D, texture_id_[i], texture_width_[i], texture_height_[i]);
    }

    // Create specific materials.
    const auto depth_idx =
        GetIndexFromCameraFlag(MLDepthCameraFlags_DepthImage);
    const auto conf_idx = GetIndexFromCameraFlag(MLDepthCameraFlags_Confidence);
    const auto ambient_idx =
        GetIndexFromCameraFlag(MLDepthCameraFlags_AmbientRawDepthImage);
    const auto raw_idx =
        GetIndexFromCameraFlag(MLDepthCameraFlags_RawDepthImage);
    shader_materials_[depth_idx] = std::make_shared<DepthMaterial>(
        texs[depth_idx], color_map_tex_, min_distance_[depth_idx],
        max_distance_[depth_idx]);
    shader_materials_[conf_idx] = std::make_shared<ConfidenceMaterial>(
        texs[depth_idx], texs[conf_idx], min_distance_[depth_idx],
        max_distance_[depth_idx]);
    shader_materials_[ambient_idx] = std::make_shared<DepthMaterial>(
        texs[ambient_idx], color_map_tex_, min_distance_[ambient_idx],
        max_distance_[ambient_idx]);
    shader_materials_[raw_idx] = std::make_shared<DepthMaterial>(
        texs[raw_idx], color_map_tex_, min_distance_[raw_idx],
        max_distance_[raw_idx]);
  ...
  void CreatePreviewNode(std::shared_ptr<Texture> legend_texture, uint8_t idx,
    ...
    gui_node->SetLocalScale(
        {1.f, -1.f * texture_height_[idx] / texture_width_[idx],
         1.f}); // Negative vertical scale to flip the image.
    ...
    auto legend_quad =
        Registry::GetInstance()->GetResourcePool()->GetMesh<QuadMesh>();
    auto legend_mat = std::make_shared<TexturedMaterial>(legend_texture);
    legend_mat->SetPolygonMode(GL_FILL);
    auto legend_renderable =
        std::make_shared<RenderableComponent>(legend_quad, legend_mat);
    ...
    legend_text_nodes_[idx] =
        ml::app_framework::CreatePresetNode(ml::app_framework::NodeType::Text);
    SetLegendText(idx);
    legend_text_nodes_[idx]->SetLocalTranslation(
        glm::vec3{-0.505f, -0.02f, 0.f});
    constexpr auto text_scale = 0.02f / 8.f;
    legend_text_nodes_[idx]->SetLocalScale(
        glm::vec3{text_scale, -text_scale, 1.f});
    ...
    const auto frame_type = GetFrameTypeFromStream(
        static_cast<MLDepthCameraStream>(camera_settings_.streams));
  ...
  void SetLegendText(uint8_t idx) {
    ...
    snprintf(legend_label, 75, "%1.1f%c%32.1f%c%30.1f%c", range_min, unit,
             (range_max - range_min) / 2.f, unit, range_max, unit);
    legend_text_nodes_[idx]
        ->GetComponent<ml::app_framework::TextComponent>()
        ->SetText(legend_label);
  }
  ...
  void SetPreviewVisibility(uint8_t idx, bool is_visible) {
    for (auto &child : preview_nodes_[idx]->GetChildren()) {
      SetComponentVisibility(
          child,
          is_visible); ///< Set visibility for nodes that are preview quads.
      for (auto &subchild : child->GetChildren()) {
        SetComponentVisibility(
            subchild,
            is_visible); ///< Set visibility for both legend-related subnodes.
      }
    }
  }
  ...
  void AcquireNewFrames() {
    const auto res = MLDepthCameraGetLatestDepthData(
        cam_context_, 0, &depth_camera_data_); ///< Passing 0 timeout to make
                                               ///< the call nonblocking.
    if (res == MLResult_Ok) {
      ...
      UNWRAP_MLRESULT(MLDepthCameraReleaseDepthData(
          cam_context_,
          &depth_camera_data_)); ///< Always remember to release the data,
                                 ///< otherwise buffer overload will happen.
    } else if (res != MLResult_Timeout) { ///< Need to check for timeout,
                                          ///< especially when nonblocking.
      UNWRAP_MLRESULT(res);
    }
  }
  ...
  void UpdateImage(MLDepthCameraData &data, MLDepthCameraFlags data_flag) {
    ...
    // Check if the frame buffer is valid (could also check
    // camera_settings_.flags)
    if (data_map == nullptr || data_map->size == 0) {
      return;
    }

    // All depth data in this app is float type, just making sure that this is
    // right and our vectors won't break.
    constexpr auto type_size = sizeof(float);
    if (data_map->bytes_per_unit != type_size) {
      ALOGE("Bytes per pixel equal to %d, instead of %ld! Data alignment "
            "mismatch!",
            data_map->bytes_per_unit, type_size);
      FinishActivity();
      return;
    }
    ...
    const auto [min, max] =
        std::minmax_element(data_float_ptr, data_float_ptr + data_size);
    min_values_[idx] = *min;
    max_values_[idx] = *max;
  }
  ...
  void UpdateFrameData(MLDepthCameraData &data) {
    ...
    GetMLTimeString(data.frames[0].frame_timestamp, last_dcam_timestamp_str_,
                    last_dcam_timestamp_str_size_);
  }
  ...
  void GetMLTimeString(const MLTime time, char *cstr, size_t size) {
    ...
    snprintf(cstr, size, "%lld:%02lld:%02lld", hours_div.quot, mins_div.quot,
             mins_div.rem);
  }
  ...
  void CheckFrameNumber(MLDepthCameraData &data) {
    ...
    if (last_dcam_frame_number_ >= 0) {
      ...
      if (diff > 1) {
        // ALOGV("Lost frames! Current frame: %ld, last frame: %ld, difference:
        // %ld.", current_frame_number, last_dcam_frame_number_, diff);
      }
    }
  ...
  void SetNewFrame(uint8_t idx, void *data) {
    ...
    glTexImage2D(GL_TEXTURE_2D, 0, GL_R32F, texture_width_[idx],
                 texture_height_[idx], 0, GL_RED, GL_FLOAT, data);
    glBindTexture(GL_TEXTURE_2D, 0);
  }
  ...
  MLDepthCameraFlags GetCameraFlagFromIndex(uint8_t idx) {
    ...
    case 1:
      return MLDepthCameraFlags_Confidence;
    case 2:
      return MLDepthCameraFlags_AmbientRawDepthImage;
    case 3:
      return MLDepthCameraFlags_RawDepthImage;
    default:
  ...
  MLDepthCameraFrameBuffer *GetCameraFrameBuffer(MLDepthCameraData &data,
                                                 MLDepthCameraFlags flag) {
    if (data.frame_count == 0) {
      return nullptr;
    }
    switch (flag) {
    case MLDepthCameraFlags_DepthImage:
      return data.frames[0].depth_image;
    case MLDepthCameraFlags_Confidence:
      return data.frames[0].confidence;
    case MLDepthCameraFlags_AmbientRawDepthImage:
      return data.frames[0].ambient_raw_depth_image;
    case MLDepthCameraFlags_RawDepthImage:
      return data.frames[0].raw_depth_image;
    default:
      ALOGE("Unsupported flag used, can't return valid frame buffer! Returning "
            "nullptr.");
      return nullptr;
    }
  }
  ...
  MLDepthCameraFrameType GetFrameTypeFromStream(MLDepthCameraStream stream) {
    ...
    case MLDepthCameraStream_ShortRange:
      return MLDepthCameraFrameType_ShortRange;
    default:
  ...
  MLDepthCameraStream
  GetStreamFromFrameType(MLDepthCameraFrameType frame_type) {
    switch (frame_type) {
    case MLDepthCameraFrameType_LongRange:
      return MLDepthCameraStream_LongRange;
    case MLDepthCameraFrameType_ShortRange:
      return MLDepthCameraStream_ShortRange;
    default:
  ...
  void StartUpdateSettingsAsync() {
    update_settings_future_ =
        std::async(std::launch::async, MLDepthCameraUpdateSettings,
                   cam_context_, &camera_settings_);
  }
  ...
  void UpdateStreamCapabilities() {
    ...
    for (int frame_type = 0; frame_type < MLDepthCameraFrameType_Count;
         frame_type++) {
      const auto stream = GetStreamFromFrameType(
          static_cast<MLDepthCameraFrameType>(frame_type));
      filter.streams = stream;
      UNWRAP_MLRESULT(
          MLDepthCameraGetCapabilities(cam_context_, &filter, &cap_list));
      ...
      for (size_t i = 0; i < cap_list.size; i++) {
        ...
        // We're quering only single Stream at a time via filter.streams, so it
        // will have at max 1 elem
        ...
        const char *frame_rate_str =
            GetMLDepthCameraFrameRateString(stream_cap.frame_rate);
      ...
      UNWRAP_MLRESULT(
          MLDepthCameraReleaseCapabilities(cam_context_, &cap_list));
    }
  }

  int FindFrameRateIdx(MLDepthCameraFrameType frame_type,
  ...
  std::array<std::vector<MLDepthCameraStreamCapability>,
             MLDepthCameraFrameType_Count>
      stream_caps_;
  std::array<std::vector<const char *>, MLDepthCameraFrameType_Count>
      frame_rates_;
  int8_t cap_idx_;
  ...
  float capture_max_pair_dist_mm_ =
      200.f; // reject frame if any pair exceeds this
  ...
  std::vector<cv::Vec3f>
  AlignFrameToReference(const std::vector<cv::Vec3f> &src,
  ...
  void FinalizeCapture() {
    ...
    for (int iter = 0; iter < NUM_ITERATIONS; ++iter) {
      ...
      for (int f = 0; f < F; ++f) {
        std::vector<cv::Vec3f> aligned =
            AlignFrameToReference(capture_frame_positions_[f], reference);
        for (int i = 0; i < N; ++i)
          sum[i] += aligned[i];
      }
      for (int i = 0; i < N; ++i)
    ...
    for (int i = 0; i < N; ++i)
      capture_result_mm_.at<cv::Vec3f>(i, 0) =
          (reference[i] - centroid) * 1000.f;

    char buf[256];
    snprintf(
        buf, sizeof(buf),
        "Captured %d spheres over %d frames (max std: %.2f mm, skipped: %d)", N,
        F, capture_max_std_mm_, capture_frames_skipped_);
