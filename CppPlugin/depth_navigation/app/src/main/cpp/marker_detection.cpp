...
#define MARKER_DETECTION_LOG(...)                                              \
  __android_log_print(ANDROID_LOG_VERBOSE, "MarkerDetection", __VA_ARGS__)
#define MARKER_DEPTHLOG(...)                                                   \
  __android_log_print(ANDROID_LOG_INFO, "MarkerDepth", __VA_ARGS__)
#define MARKER_PAIRLOG(...)                                                    \
  __android_log_print(ANDROID_LOG_INFO, "MarkerPair", __VA_ARGS__)
...
namespace {
constexpr float kDepthBaselineEps = 1e-6f;
} // namespace
...
std::vector<DetectedMarker> MarkerDetection::detectMarkerPositions(
    float *raw_intensity_buffer, float *calibrated_depth_buffer,
    float *ambient_intensity_buffer, int width, int height,
  ...
  const int log_every_n = std::max(1, config.log_every_n_frames);
  const bool should_log_depth_scaling =
      config.log_depth_scaling &&
      ((frame_index % static_cast<uint64_t>(log_every_n)) == 0);
  const int log_pair_n = std::max(1, config.log_pairwise_every_n_frames);
  const bool should_log_pairwise =
      config.log_pairwise_distances &&
      ((frame_index % static_cast<uint64_t>(log_pair_n)) == 0);
  ...
  if (config.use_ambient_subtraction && ambient_intensity_buffer != nullptr) {
    ...
    MARKER_DETECTION_LOG(
        "[DEBUG] Using ambient subtraction for marker detection");
  } else {
  ...
  MARKER_DETECTION_LOG("[DEBUG] Intensity range: min=%.2f, max=%.2f", min_val,
                       max_val);
  ...
  int gk =
      std::max(1, config.gaussian_blur_kernel_size | 1); // ensure odd, >= 1
  if (gk > 1) {
  ...
  MARKER_DETECTION_LOG("[DEBUG] Bright pixels after threshold: %d (%.2f%%)",
  ...
  // This helps when a single physical marker appears as several tiny islands.
  {
    ...
    cv::Mat element =
        cv::getStructuringElement(cv::MORPH_ELLIPSE, cv::Size(ks, ks));
    cv::morphologyEx(binary_8bit, binary_8bit, cv::MORPH_CLOSE, element);
    int bright_pixels_after = cv::countNonZero(binary_8bit);
    MARKER_DETECTION_LOG("[DEBUG] Bright pixels after morphology: %d",
                         bright_pixels_after);
  }

  // Find blobs and convert to 3D
  std::vector<DetectedMarker> markers =
      findMarkerBlobs(binary_8bit, intensity_image, calibrated_depth_buffer,
                      width, height, intrinsics, config, rejected_blobs,
                      should_log_depth_scaling, frame_index);

  if (should_log_pairwise) {
    int n = static_cast<int>(markers.size());
    if (n >= 2) {
      for (int i = 0; i < n; ++i) {
        for (int j = i + 1; j < n; ++j) {
          float dist_mm = (float)cv::norm(markers[i].position_camera -
                                          markers[j].position_camera) *
                          1000.f;
          MARKER_PAIRLOG("frame=%llu pair(%d,%d)=%.1fmm",
                         static_cast<unsigned long long>(frame_index), i, j,
                         dist_mm);
        }
      }
    } else {
      MARKER_PAIRLOG("frame=%llu markers=%d (no pairs)",
                     static_cast<unsigned long long>(frame_index), n);
    }
  }
...
void MarkerDetection::pixelToCameraPlane(
    float u, float v, const MLDepthCameraIntrinsics &intrinsics, float &x,
    float &y) {
  // Build camera matrix from intrinsics
  cv::Mat camera_matrix =
      (cv::Mat_<double>(3, 3) << intrinsics.focal_length.x, 0.0,
       intrinsics.principal_point.x, 0.0, intrinsics.focal_length.y,
       intrinsics.principal_point.y, 0.0, 0.0, 1.0);

  // Distortion coefficients [k1, k2, p1, p2, k3]
  cv::Mat dist_coeffs =
      (cv::Mat_<double>(5, 1) << intrinsics.distortion[0], // k1
  ...
  cv::undistortPoints(input_points, undistorted_points, camera_matrix,
                      dist_coeffs);
...
float MarkerDetection::expectedMarkerAreaPixels(float depth_m, float radius_mm,
                                                float focal_x_px,
                                                float focal_y_px) {
  // A_px ≈ π·r²·fx·fy / d²
  // r, d in mm; fx, fy in pixels (intrinsics). depth_m in meters -> d_mm =
  // depth_m * 1000
  ...
  MARKER_DETECTION_LOG("[DEBUG] Depth in m and cm: depth_m=%.3f, d_cm=%.3f",
                       depth_m, d_cm);
  float d2_mm2 = d_mm * d_mm;
  float r2_mm2 = radius_mm * radius_mm;
  float area =
      static_cast<float>(M_PI) * r2_mm2 * focal_x_px * focal_y_px / d2_mm2;
...
std::vector<DetectedMarker> MarkerDetection::findMarkerBlobs(
    const cv::Mat &binary_image, const cv::Mat &intensity_image,
    float *calibrated_depth_buffer, int width, int height,
    const MLDepthCameraIntrinsics &intrinsics,
    const MarkerDetectionConfig &config,
    std::vector<RejectedBlob> *rejected_blobs, bool should_log_depth_scaling,
    uint64_t frame_index) {
  static bool baseline_latched = false;
  static float z_initial = 0.f;
  static float r_initial = 0.f;

  if (config.reset_depth_baseline) {
    baseline_latched = false;
  }
  ...
  int num_components = cv::connectedComponentsWithStats(binary_image, labels,
                                                        stats, centroids, 8);

  MARKER_DETECTION_LOG("[DEBUG] Found %d connected components",
                       num_components - 1);
  ...
  for (int i = 1; i < num_components; i++) {
    ...
    // Get centroid pixel coordinates first (needed for depth lookup and
    // distance-based area)
    ...
    if (u < 0 || u >= width || v < 0 || v >= height) {
      MARKER_DETECTION_LOG(
          "[DEBUG] Rejected blob %d: centroid out of bounds (%.1f, %.1f)", i, u,
          v);
      if (rejected_blobs) {
        RejectedBlob blob;
        blob.centroid_pixel =
    ...
    if (depth_m <= 0.0f || std::isnan(depth_m) || std::isinf(depth_m)) {
      MARKER_DETECTION_LOG("[DEBUG] Rejected blob %d: invalid depth=%.3f", i,
                           depth_m);
      if (rejected_blobs) {
        RejectedBlob blob;
        blob.centroid_pixel =
        ...
        blob.intensity =
            intensity_image.at<float>(static_cast<int>(v), static_cast<int>(u));
        blob.expected_area = 0.0f;
        blob.expected_area_min = 0.0f;
        blob.expected_area_max = 0.0f;
    ...
    // Filter by blob area: A_px ≈ π·r²·fx·fy/d² (expected area from distance
    // and radius)
    float expected = expectedMarkerAreaPixels(depth_m, config.sphere_radius_mm,
    ...
    if (expected <= 0.f || area < min_expected || area > max_expected) {
      MARKER_DETECTION_LOG("[DEBUG] Rejected blob %d: area=%d (expected %.1f, "
                           "range [%.1f, %.1f]), depth=%.3fm",
    ...
    MARKER_DETECTION_LOG(
        "[DEBUG] Blob %d: centroid=(%.1f,%.1f) depth=%.3fm area=%d (expected "
        "%.1f, range [%.1f, %.1f]), depth= %.3fm",
        i, u, v, depth_m, area, expected, min_expected, max_expected, depth_m);

    // Convert pixel to camera plane
    float x, y;
    pixelToCameraPlane(static_cast<float>(u), static_cast<float>(v), intrinsics,
                       x, y);

    // The depth buffer corresponds to the visible sphere surface. Add marker
    // radius to estimate the sphere-center depth used for 3D reconstruction.
    ...
    float range_m = cv::norm(position_3d);
    float lateral_m = std::sqrt(position_3d[0] * position_3d[0] +
                                position_3d[1] * position_3d[1]);
    float off_axis_deg = std::atan2(lateral_m, position_3d[2]) * 180.0f /
                         static_cast<float>(M_PI);

    if (should_log_depth_scaling) {
      MARKER_DEPTHLOG(
          "[SCALE] frame=%llu blob=%d uv=(%.1f,%.1f) d_raw=%.3f d_corr=%.3f "
          "xyz=(%.3f,%.3f,%.3f) z=%.3f r=%.3f off_axis=%.2fdeg area=%d "
          "exp=%.1f",
          static_cast<unsigned long long>(frame_index), i, u, v, depth_m,
          depth_corrected, position_3d[0], position_3d[1], position_3d[2],
          position_3d[2], range_m, off_axis_deg, area, expected);
    }
    ...
    min_z = std::min(min_z, position_3d[2]);
    max_z = std::max(max_z, position_3d[2]);
    sum_z += position_3d[2];
    min_range = std::min(min_range, range_m);
    max_range = std::max(max_range, range_m);
    sum_range += range_m;
  }

  if (should_log_depth_scaling) {
    if (!markers.empty()) {
      const float count = static_cast<float>(markers.size());
      const float z_mean = sum_z / count;
      const float r_mean = sum_range / count;

      if (!baseline_latched) {
        z_initial = z_mean;
        r_initial = r_mean;
        baseline_latched = true;
      }
      ...
      float ratio_r = 0.f;
      float dr = 0.f;
      if (baseline_latched && z_initial > kDepthBaselineEps) {
        ratio_z = z_mean / z_initial;
        dz = z_mean - z_initial;
      }
      if (baseline_latched && r_initial > kDepthBaselineEps) {
        ratio_r = r_mean / r_initial;
        dr = r_mean - r_initial;
      }

      MARKER_DEPTHLOG(
          "[SCALE] frame=%llu markers=%zu z[min/avg/max]=%.3f/%.3f/%.3f "
          "r[min/avg/max]=%.3f/%.3f/%.3f "
          "z0=%.3f r0=%.3f ratio_z=%.4f dz=%.4fm ratio_r=%.4f dr=%.4fm",
          static_cast<unsigned long long>(frame_index), markers.size(), min_z,
          z_mean, max_z, min_range, r_mean, max_range, z_initial, r_initial,
          ratio_z, dz, ratio_r, dr);
    } else {
      MARKER_DEPTHLOG("[SCALE] frame=%llu markers=0 z0=n/a ratio=n/a dz=n/a "
                      "(baseline unchanged)",
