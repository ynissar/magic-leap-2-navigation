...
// Positions are in metres (ML2 native). Converted to mm at the tool_tracking
// boundary (tool_tracking.cpp:44-48). Sphere-radius correction is applied by
// adding radius to centroid depth to estimate sphere center depth. Defaults
// tuned for retroreflective IR markers. Adjustable at runtime via GUI sliders.
...
struct MarkerDetectionConfig {
  float intensity_threshold_min =
      2000.0f; // Working defaults for current marker set
  float intensity_threshold_max = 3500.0f;
  float sphere_radius_mm = 5.0f; // Default for current marker set; adjustable
                                 // at runtime via GUI sliders.
  ...
  // Gaussian blur kernel size applied before thresholding (must be odd; 1 =
  // disabled)
  ...
  // Emit pairwise inter-marker distance logs (MarkerPair tag) every N detector
  // calls.
  bool log_pairwise_distances = false;
  int log_pairwise_every_n_frames = 10;

  // Clear session baseline for depth-scale ratio (mean z / z_initial). Set from
  // UI on "Log depth scaling" rising edge or "Reset depth baseline" button.
...
public:
   ...
   * @param calibrated_depth_buffer Calibrated depth data in meters from
   * depth_image
   * @param ambient_intensity_buffer Optional ambient raw data from
...
private:
   ...
   */
  static void pixelToCameraPlane(float u, float v,
                                 const MLDepthCameraIntrinsics &intrinsics,
                                 float &x, float &y);

  /**
   * Expected marker area in pixels from A_px ≈ π·r²·fx·fy/d² (r,d in mm; fx,fy
   * in pixels). Used when use_distance_based_area is true to filter blobs by
   * expected size at given depth.
   */
  static float expectedMarkerAreaPixels(float depth_m, float radius_mm,
   ...
   * @param intensity_image Original intensity image for centroid intensity
   * lookup
   ...
   * @return Vector of detected markers
   */
  static std::vector<DetectedMarker>
  findMarkerBlobs(const cv::Mat &binary_image, const cv::Mat &intensity_image,
                  ...
                  bool should_log_depth_scaling, uint64_t frame_index);
};
