...
static void LogToolDefinition(const TrackedTool &tool) {
  TOOL_DEF_LOG(
      "=== tool: %s  spheres: %d  radius: %.1f mm ===", tool.identifier.c_str(),
      tool.num_spheres, tool.sphere_radius_mm);

  // Sphere positions.
  for (int i = 0; i < tool.num_spheres; ++i) {
    const cv::Vec3f &p = tool.spheres_xyz_mm.at<cv::Vec3f>(i, 0);
    TOOL_DEF_LOG("  sphere[%d]  x=%.3f  y=%.3f  z=%.3f  (mm)", i, p[0], p[1],
                 p[2]);
  }
  ...
  for (int i = 0; i < n; ++i) {
    for (int j = i; j < n; ++j) {
      ...
      if (out_sides.empty() || dist >= out_sides.back().distance) {
        ...
        auto it = std::upper_bound(out_sides.cbegin(), out_sides.cend(), s,
                                   &Side::compare);
...
bool ToolTracker::AddTool(cv::Mat3f spheres_mm, float radius_mm,
  ...
  tool.min_visible_spheres =
      std::max(3, std::min(min_visible, tool.num_spheres));
  ...
  for (int i = 0; i < tool.num_spheres; ++i)
    tool.sphere_kalman_filters.emplace_back(tool.kalman_measurement_noise,
  ...
  ALOGI("[ToolTracker] Added tool '%s' (index=%d, spheres=%d, r=%.1fmm, "
        "min_visible=%d)",
...
bool ToolTracker::RemoveTool(const std::string &identifier) {
  ...
  ALOGI("[ToolTracker] Removing tool '%s' (index=%d)", identifier.c_str(),
        removed_index);
...
cv::Mat ToolTracker::GetToolTransform(const std::string &identifier) const {
  ...
  auto it = tool_index_.find(identifier);
  if (it == tool_index_.end()) {
    ALOGW("[ToolTracker] GetToolTransform: unknown tool '%s'",
          identifier.c_str());
    return cv::Mat::zeros(8, 1, CV_32F);
  }
  ...
  ALOGV(
      "[ToolTracker] GetToolTransform('%s'): ever_tracked=%d, valid_flag=%.1f",
      identifier.c_str(), t.ever_tracked ? 1 : 0,
      t.cur_transform.at<float>(7, 0));
