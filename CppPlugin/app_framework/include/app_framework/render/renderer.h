...
struct CameraUBO {
  CameraUBO(const glm::mat4 &in_view_proj, const glm::vec3 &in_camera_position) :
...
struct Light {
  Light() {}
  Light(const glm::vec3 &in_light_position, const glm::vec3 &in_light_color, const glm::vec3 &in_light_direction,
        LightType type, float strength) :
...
struct LightsUBO {
  LightsUBO(const std::vector<Light> &in_lights_array) {
    int lightCnt = in_lights_array.size() > LightComponent::MAXIMUM_LIGHTS ? LightComponent::MAXIMUM_LIGHTS :
                                                                             in_lights_array.size();
...
public:
  ...
  void SetPreRenderCameraCallback(const std::function<void(std::shared_ptr<CameraComponent>)> &callback) {
    pre_cam_callback_ = callback;
  }
  void SetPostRenderCameraCallback(const std::function<void(std::shared_ptr<CameraComponent>)> &callback) {
    post_cam_callback_ = callback;
  }
  void SetPreRenderCallback(const std::function<void()> &callback) {
    pre_render_callback_ = callback;
  }
  void SetPostRenderCallback(const std::function<void()> &callback) {
...
private:
  ...
  using RenderableGraph =
      std::unordered_map<std::shared_ptr<Material>, std::vector<std::shared_ptr<RenderableComponent>>>;
  ...
  GLuint camera_uniform_buffer_;
  GLuint light_uniform_buffer_;
  GLuint model_uniform_buffer_;

  std::vector<Light> lights_;
  bool camera_uniform_buffer_dirty_;
};
