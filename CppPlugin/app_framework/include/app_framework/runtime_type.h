...
static inline constexpr uint64_t _HashStrWithFNVInternal(uint64_t hash, const char *str, int32_t iteration,
                                                         uint64_t size) {
  return iteration < (int64_t)size ?
...
public:                                                                                             \
  static const constexpr RUNTIME_TYPE_ID RuntimeTypeId = _HashStrWithFNV(#type, sizeof(#type) - 1); \
