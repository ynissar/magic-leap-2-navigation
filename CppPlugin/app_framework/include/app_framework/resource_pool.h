...
public:
  ...
  void InitializePresetResources(ml::IAssetManagerPtr asset_manager);
  ...
  template <typename ProgramType>
  std::shared_ptr<ProgramType> LoadShaderFromCode(const std::string &code,
...
private:
  ...
  ml::IAssetManagerPtr asset_manager_;
  // Load a model from a 3D file and cache it, the returned material instance will always be a new one
