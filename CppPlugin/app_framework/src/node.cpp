...
bool Node::AddChild(std::shared_ptr<ml::app_framework::Node> new_child) {
  ...
  while (!dfs_stack.empty()) {
    ...
    if (back == shared_this) {
      ALOGE("Adding node \"%s\" as a child of node \"%s\" would create a cycle", new_child->name_.c_str(),
            name_.c_str());
      return false;
    }
    dfs_stack.pop_back();
...
bool Node::RemoveChild(std::shared_ptr<ml::app_framework::Node> child) {
  if (child) {
    child_list_.erase(std::remove_if(child_list_.begin(), child_list_.end(),
                                     [child](std::shared_ptr<ml::app_framework::Node> &node) {
                                       return node == child;
                                     }),
                      child_list_.end());
    child->parent_.reset();
  } else {
    ALOGE("Can not remove null pointer");
    return false;
  }
  return true;
}
...
void Node::SetLocalPose(const Pose &pose) {
  SetLocalTranslation(pose.position);
  SetLocalRotation(pose.rotation);
}

void Node::SetWorldPose(const Pose &pose) {
  SetWorldTranslation(pose.position);
  SetWorldRotation(pose.rotation);
}
...
Pose Node::GetLocalPose() const {
  return {GetLocalRotation(), GetLocalTranslation()};
}

Pose Node::GetWorldPose() const {
  return {GetWorldRotation(), GetWorldTranslation()};
}
