...
Program::Program(const char *code, GLenum type) : program_(0), type_(type), uniform_cnt_(0), uniform_blk_cnt_(0) {
  ...
  if (!success) {
    ...
    ALOGA("Shader compilation failed: %s", info_log);
  }
  ...
  for (int32_t i = 0; i < uniform_blk_cnt_; ++i) {
    ...
    for (int32_t j = 0; j < active_uniforms; j++) {
      ...
      ALOGD("Found uniform member: name(%s), index(%u), size(%" PRIu64 "), offset(%" PRIu64 ") type(%x)",
            member.name.c_str(), member.index, member.size, member.offset, member.type);

      data.entries[j] = member;
    }
    uniform_blocks_by_name_[data.name] = data;

    ALOGD("Found uniform: name(%s), index(%u), size(%" PRIu64 "), binding(%u)", data.name.c_str(), data.index,
          data.size, data.binding);
  }
  ...
  for (int32_t i = 0; i < uniform_cnt_; ++i) {
    ...
    ALOGD("Found uniform: name(%s), index(%u), size(%" PRIu64 "), type(%x)", data.name.c_str(), data.index, data.size,
          data.type);
