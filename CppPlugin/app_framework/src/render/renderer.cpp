...
Renderer::Renderer() :
    program_pipeline_(0),
    camera_uniform_buffer_(0),
    light_uniform_buffer_(0),
    model_uniform_buffer_(0),
...
void Renderer::Render() {
  ...
  for (const std::shared_ptr<CameraComponent> &cam : queued_cameras_) {
    ...
    if (blit_target) {
      render_target->BindTargetForRead();
      blit_target->BindTargetForDraw();
