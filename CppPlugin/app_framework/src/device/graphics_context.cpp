...
GraphicsContext::GraphicsContext(const char *, bool, int32_t, int32_t) : close_callback_(nullptr) {
  ...
  EGLBoolean b = eglChooseConfig(display_handle_, config_attribs, &egl_config, 1, &config_size);
  if (!b) {
    ALOGE("eglChooseConfig failed");
  }

  EGLint context_attribs[] = {EGL_CONTEXT_MAJOR_VERSION_KHR, 4, EGL_CONTEXT_MINOR_VERSION_KHR, 5, EGL_NONE};
  context_handle_ = eglCreateContext(display_handle_, egl_config, EGL_NO_CONTEXT, context_attribs);

  if (context_handle_ == EGL_NO_CONTEXT) {
...
void GraphicsContext::UnMakeCurrent() {
  eglMakeCurrent(display_handle_, EGL_NO_SURFACE, EGL_NO_SURFACE, EGL_NO_CONTEXT);
}
...
GraphicsContext::~GraphicsContext() {
  if (context_handle_ != EGL_NO_CONTEXT) {
    ...
    context_handle_ = EGL_NO_CONTEXT;
  }
  if (display_handle_ != EGL_NO_DISPLAY) {
    eglTerminate(display_handle_);
    display_handle_ = EGL_NO_DISPLAY;
  }
}
